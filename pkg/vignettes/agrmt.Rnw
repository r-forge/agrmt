...
%\VignetteIndexEntry{R Package to Calculate Agreement, Consensus, Dispersion, and Polarization}
...
This package calculates agreement (consensus, concentration, polarization, dispersion) in ordered rating scales. It implements Cees van der Eijk's \citeyearpar{vdE2001} algorithmic measure of agreement `A', which can be used to describe agreement, consensus, or polarization among respondents. Several other measures of agreement or consensus are included, as is Galtung's \citeyearpar{Galtung} AJUS system.
