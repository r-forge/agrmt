...
This package calculates agreement (consensus, polarization, dispersion) in ordered rating scales. It principally implements Cees van der Eijk's \citeyearpar{vdE2001} measure of agreement ``A'', which can be used to describe agreement, consensus, or polarization among respondents. Other measures of agreement or consensus are included, as is Galtung's \citeyearpar{Galtung} AJUS system.
...
Ordered rating scales are a common format in surveys, and often we are interested in the extent to which responses are in agreement (whether there is consensus, whether there is concentration among the responses). Although frequently used, standard deviations are in this case an inappropriate measure. Van der Eijk's measure of agreement ``A'' dis-aggregates frequency distributions into component parts called layers. At the level of these layers, agreement can easily be determined, and the measure of agreement provides the weighted average.
